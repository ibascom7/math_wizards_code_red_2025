...
import { KeywordDetector, Word, DetectorConfig } from './KeywordDetector';
import { getAllFields, hasField } from './fieldKeywords';
    ...
    endpoints: {
      'POST /detect': {
        ...
        body: {
          ...
          minConfidence:
            'number (optional) - Minimum confidence threshold (0-1)',
...
async function handleDetect(request: Request): Promise<Response> {
  ...
  for (let i = 0; i < body.words.length; i++) {
    ...
    if (!word.text || typeof word.text !== 'string') {
      return errorResponse(
        `Invalid word at index ${i}: missing or invalid "text" field`
      );
    }
    if (typeof word.x !== 'number' || typeof word.y !== 'number') {
      return errorResponse(
        `Invalid word at index ${i}: missing or invalid position (x, y)`
      );
    }
    if (typeof word.width !== 'number' || typeof word.height !== 'number') {
      return errorResponse(
        `Invalid word at index ${i}: missing or invalid dimensions (width, height)`
      );
    }
  }
  ...
  if (body.targetFields) {
    ...
    if (invalidFields.length > 0) {
      return errorResponse(
        `Invalid fields: ${invalidFields.join(', ')}. Use GET /fields to see available fields.`
      );
    }
  }
  ...
  try {
    ...
    const detector = new KeywordDetector(config);
    ...
    return errorResponse(`Detection error: ${message}`, 500);
  }
}
