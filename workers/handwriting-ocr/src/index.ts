 ...
 * Part of Math Wizards - Used as preprocessing step before Mathpix OCR
 */
...
export default {
  async fetch(
    ...
    try {
      ...
      // Support both FormData (file upload) and JSON (base64) formats
      if (contentType.includes('application/json')) {
        // JSON format: { "image": "base64string" }
        const body = await request.json() as { image: string };

        if (!body.image) {
          return jsonResponse(
            {
              error: 'No image provided. Send as JSON with key "image" or form-data',
              success: false,
            },
            400
          );
        ...
        base64 = body.image;
        console.log('Processing base64 image from JSON, size:', base64.length);

      } else {
        // FormData format: image file
        ...
        if (!image) {
          return jsonResponse(
            {
              error: 'No image provided. Send as form-data with key "image" or JSON',
              success: false,
            },
            400
          );
        ...
        console.log('Processing image file:', image.name, image.type);
